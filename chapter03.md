...
* 0170000(S_IFMT) : 파일의 종류 비트를 가져오기 위한 **비트 마스크**
    * 017(8) = 111(2)이므로 AND연산을 하면 st_mode의 값과 AND 연산을 하면 파일 종류 부분만 남음
...
* 비트 마스크(S_IFMT)와 AND 연산 결과를 상수와 비교해 파일의 종류를 판단함
...
* 심벌릭 링크를 인자로 지정한 경우 원본 파일이 아닌 심벌릭 링크 파일이 삭제됨
